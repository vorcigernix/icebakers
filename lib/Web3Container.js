...
import { claimTip, connectWallet } from "./tipsService";
...
const Web3Container = (props) => {
  ...
  const { kit, address, updateNetwork, sendTransaction, updateKit } = useContractKit();
  const [session, setSession] = useState(props.session);
  const [connected, setConnected] = useState(session && session.user.address);
  ...
  useEffect(async e => {
    ...
    await claimTip(kit, sendTransaction, address, json);
    setSession(await getSession());
  }, [address, isConnecting, modalIsOpen]);
