...
export default class Web3Container extends React.Component {
  ...
  componentDidMount() {
    const connected = this.props.session && this.props.session.user && this.props.session.user.address;
  ...
  render() {
    ...
    if (!walletInstalled) {
      return (
        <div className="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-green-50">
          <a href="https://metamask.io/" target="metamask">Please install Metamask</a>
        </div>
      );
    }
    if (!connected) {
      return (
        <>
          ...
          >
            ...
            Connect Wallet {this.props.claimTip ? " to Claim Tips": ""}
          </button>
        </>
      );
    }
    ...
    return web3 && accounts
      ? this.props.render({ web3, accounts, contract, session: this.state.session ? this.state.session : this.props.session })
